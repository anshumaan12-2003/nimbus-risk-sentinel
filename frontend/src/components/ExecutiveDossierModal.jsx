...
function Report() {
  ...
  return (
    <article className="mx-auto max-w-[820px] px-10 py-10 text-fg print:max-w-none print:px-0 print:py-0">
      <header className="flex items-start justify-between gap-6 border-b border-line pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-fg-2">Nimbus · AWS {pre.data?.account_id || latest.account_id}</p>
          </div>
        </div>
      ...
      {open.length > top.length && <p className="mt-2 text-xs text-fg-3">+{open.length - top.length} more in Nimbus.</p>}

      <footer className="mt-10 border-t border-line pt-4 text-2xs text-fg-3">
        Generated by Nimbus from scan {latest.id.slice(0, 8)}. Fixes are applied only after a second person approves them, and every change is recorded with a rollback.
      </footer>
    </article>
  )
}
